...
class SingleWriterShmRingBuffer:
        ...
        if create:
            ...
            with patch(
                ...
                try:
                    ...
                    pass
        logger.info(
            f"Shared memory created/opened with name: {self.shared_memory.name}, size: {self.data_buffer_size}"
        )
    ...
    def allocate_buf(self, size: int) -> tuple[int, int]:
        ...
        size += self.metadata_size  # add metadata size to the size of the data
        # reset to beginning if the buffer does have enough contiguous space
        if self.data_buffer_end % self.data_buffer_size + size > self.data_buffer_size:
            self.data_buffer_end = (
                self.data_buffer_end // self.data_buffer_size + 1
            ) * self.data_buffer_size

        # check if we have enough space in the data buffer
        # i.e. if the new end (self.data_buffer_end + size) exceeds the start of the data buffer
        if self.data_buffer_end + size - self.data_buffer_start > self.data_buffer_size:
            raise MemoryError(
                "Not enough space in the data buffer, try calling try_free_buf() to free up space"
            )

        # first 4 bytes as the monotonic id
        buf_idx = self.data_buffer_end % self.data_buffer_size
        self.shared_memory.buf[buf_idx : buf_idx + 4] = self.monotonic_id_end.to_bytes(
            4, "little", signed=True
        )
        # next 4 bytes as the size of the data buffer
        self.shared_memory.buf[buf_idx + 4 : buf_idx + 8] = size.to_bytes(
            4, "little", signed=True
        )
    ...
    @contextmanager
    def access_buf(self, address: int) -> tuple[memoryview, memoryview]:
        buf_idx = address % self.data_buffer_size

        # read metadata
        metadata_buff = self.shared_memory.buf[buf_idx : buf_idx + self.metadata_size]
        ...
        data_buff = self.shared_memory.buf[
            buf_idx + self.metadata_size : buf_idx + size
        ]
        with (memoryview(data_buff) as data_view,):
            yield data_view, (id, size)

    def free_buf(self) -> None:
        # free the buffer by resetting the metadata
        # this is a no-op in shared memory, but we need to keep track of the metadata
...
class SingleWriterShmObjectStorage:
    ...
    Supports put(key, value) and get(key) operations with automatic serialization.
    """
    ...
    def __init__(
        ...
        if not self.is_writer and reader_lock is None:
            raise ValueError(
                "Lock must be provided for readers, as they need to synchronize access."
            )
        ...
        self.key_index: Dict[str, Tuple[int, int]] = (
            {}
        )  # key -> (address, monotonic_id)
        self.id_index: Dict[int, str] = {}  # monotonic_id -> key
        self.encoder = MsgpackEncoder()
        self.mm_decoder = MsgpackDecoder(MultiModalKwargs)
        self.tensor_decoder = MsgpackDecoder(torch.Tensor)

    def calcsize_serielize(self, value: Any) -> Tuple[int, Union[bytes, np.ndarray]]:

        len_arr = None
        try:
            # if isinstance(value, MultiModalKwargs):
            if isinstance(value, torch.Tensor) or isinstance(value, MultiModalKwargs):
                type_name = type(value).__name__
                # For MultiModalKwargs, serialize directly
    ...
    def put(self, key: str, value: Any) -> None:
        ...
        object_data, data_bytes, object_metadata, md_bytes = self.calcsize_serielize(
            value
        )
    ...
    def get(self, address: int, monotonic_id: int) -> Any:
        ...
        with self.ring_buffer.access_buf(address) as (data_view, buf_metadata):
            ...
            if buf_metadata[0] != monotonic_id:
                raise ValueError(
                    f"Data for address:id '{address}:{monotonic_id}' has been modified or is invalid"
                )

            # pickle.loads do not read past the end of a pickled object within a large buffer
            type_name, nbytes, len_arr = pickle.loads(data_view[self.flag_bytes :])
            ...
            # decrease the in-use flag
            with self._reader_lock:
                data_view[: self.flag_bytes] = (
                    int.from_bytes(data_view[: self.flag_bytes], "little", signed=True)
                    - 1
                ).to_bytes(self.flag_bytes, "little", signed=True)
    ...
    def create_from_handle(
        ...
        """
        logger.info(f"Creating storage from handle: {handle}")
            ...
            for request_data in scheduler_output.scheduled_new_reqs:
                for i in range(len(request_data.mm_inputs)):
                    ...
                    if "address" in mm_input:
                        address, monotonic_id = mm_input["address"], mm_input["monotonic_id"]
                        request_data.mm_inputs[i] = self.get(address, monotonic_id)
