...
class MirroredProcessingCache:
    ...
    def __init__(self, vllm_config, is_writer: bool = False):
        ...
        Args:
            ...
            is_writer: Whether this instance is the writer (P0) or reader (P1), only used for shared memory cache.
        """
        ...
        if self.use_cache:
            self.mm_preprocessor_cache_type = mm_config.mm_preprocessor_cache_type
            if self.mm_preprocessor_cache_type == "shm":
                self.world_size = vllm_config.parallel_config.world_size

                ring_buffer = SingleWriterShmRingBuffer(
                    data_buffer_size=VLLM_OBJECT_STORAGE_SHM_BUFFER_SIZE_MB * 1024 * 1024,
                    name=VLLM_OBJECT_STORAGE_SHM_BUFFER_NAME,
                    create=is_writer,
                )
                self.shm_cache = SingleWriterShmObjectStorage(
                    max_object_size=VLLM_OBJECT_STORAGE_MAX_OBJECT_SIZE_MB * 1024 * 1024,
                    n_readers=self.world_size,
                    ring_buffer=ring_buffer,
                    reader_lock=Lock(),
                )
            elif self.mm_preprocessor_cache_type == "lru":
                self.mm_cache = ProcessingCache.get_lru_cache(VLLM_MM_INPUT_CACHE_GIB,
                                                      MultiModalKwargs)
            else:
                raise ValueError(
                    f"Unsupported mm_preprocessor_cache_type: {self.mm_preprocessor_cache_type}. "
    ...
    def get_and_update_p0(
        ...
        if not self.use_cache:
            ...
            return mm_inputs
        elif self.mm_preprocessor_cache_type == "shm":
    ...
    def get_and_update_p1(
        ...
        if (not self.use_cache) or (self.mm_preprocessor_cache_type == "shm"):
            assert is_list_of(mm_inputs, MultiModalKwargs)
            return mm_inputs
        elif self.mm_preprocessor_cache_type == "lru":
